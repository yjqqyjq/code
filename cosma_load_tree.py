...
def load_tree(k):
   ...
   for i in tqdm(range(len(halos_id_dm))):
      ...
      coord=coord-com[id]
      mass=mass_dm[index][r<r50[id]]
      r=np.sqrt(np.sum(coord**2,axis=1))
      
      f_dm[i]+=quadrupole(coord[:,0],coord[:,1],coord[:,2],mass)
      r_dm[i]+=np.sum(np.sqrt(np.sum(coord**2,axis=1))*mass)
   ...
   for i in tqdm(range(len(halos_id_g))):
      ...
      coord=coord[r<r50[id]]#particles within r50
      coord=coord-com[id]
      mass=mass_g[index][r<r50[id]]
      r=np.sqrt(np.sum(coord**2,axis=1))
      f_g[i]+=quadrupole(coord[:,0],coord[:,1],coord[:,2],mass)
      r_g[i]+=np.sum(np.sqrt(np.sum(coord**2,axis=1))*mass)
   f.close()
def load_star(k):
   print("starting", k)
