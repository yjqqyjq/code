...
for i in tqdm(range((len(halo_ids[halo_ids<=0])))):
      ...
      particle=fn.load_regions(path,main_id[i].astype(int),2.5*main_r200[i],dm=1,g=1,s=0,coordinate=1,extra_entry={"dm":["Velocities","Masses"],"gas":["Velocities","Masses"],"stars":["Velocities","Masses"]})
...
#      print(np.min(S))
      vmean=(np.sum(((vdm*mdm).T)[rdm<0.1],axis=0)+np.sum(((vg*mg).T)[rg<0.1],axis=0))/(np.sum(mdm[rdm<0.1])+np.sum(mg[rg<0.1]))#mean velocity within 0.1 r50 as COM velocity
...
#      print(np.sqrt(6.558*m50[i]/main_r200[i]),main_r200[i])
      hdm=np.histogram2d(rdm, vdm, bins=[bins,binsv],weights=mdm)[0]
      dm[i]=hdm
...
#      gt[i]=hg
      hg=np.histogram2d(rg, vg, bins=[bins,binsv],weights=mg)[0]
      g[i]=hg
     

     
f=h5py.File(path+'2d_fgas_H.hdf5','a')
f.create_dataset("Mdm",data=dm)
f.create_dataset("Mg",data=g)
