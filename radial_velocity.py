...
for i in tqdm(range((len(halo_ids[halo_ids<=0])))):
      ...
      particle=fn.load_regions(path,main_id[i].astype(int),2.5*main_r200[i],dm=1,g=1,s=0,coordinate=1,extra_entry={"dm":["Velocities"],"gas":["Velocities","Temperatures","ElectronNumberDensities"],"stars":[]})
...
f=h5py.File(path+'2d_fgas_H_ub.hdf5','w')

f.create_dataset("Ng_T",data=gt)
#f.create_dataset("Ng_Xray",data=gx)

f.create_dataset("Ng_S",data=gs)
