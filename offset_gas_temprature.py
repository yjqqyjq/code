...
data_h=[]
mainhalo_id=halo_id[is_mainhalo*(mass>500)*(mass<600)]
...
for i in range (0,len(mainhalo_id)):
    ...
    sgi=sg.SWIFTGalaxy(dir+"colibre_with_SOAP_membership_0127.hdf5",#"snapshots/flamingo_0008.hdf5",
    ...
    z_dm[i]=np.array(sgi.dark_matter.cartesian_coordinates.z)
    ...
    x_stars[i]=np.array(sgi.stars.cartesian_coordinates.x)
    y_stars[i]=np.array(sgi.stars.cartesian_coordinates.y)
    z_stars[i]=np.array(sgi.stars.cartesian_coordinates.z)
...
Offsets_s=np.zeros(len(mainhalo_id))
...
#    Offsets_s[i]=fn.offset(x_g[i],y_g[i],z_g[i],x_stars[i],y_stars[i],z_stars[i])/radius[id]
...
title="M>1e10"
boxused="Colibre/L0012N0094/"
np.savetxt("/home/jyang/data/"+boxused+title+".txt",np.array([S,Offsets,Center_diff]))
...
fig.savefig("/home/jyang/plot/"+boxused+"/S.png")
plt.close()
...
fig = plt.figure()
ax=plt.subplot(1,1,1)
h=ax.hist(Offsets, bins=100)
ax.set_xlabel("Offsets")
ax.set_ylabel("Counts")
ax.set_title(title)
#fig.savefig("/home/jyang/plot/Flamingo/L0200N0360/Offset_small.png")
fig.savefig("/home/jyang/plot/"+boxused+"/Offset.png")
plt.close()

fig = plt.figure()
ax=plt.subplot(1,1,1)
h=ax.hist(Center_diff, bins=100)
